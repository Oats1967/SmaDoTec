...
bool TMsgSocket::CanWrite(int Timeout)
{
    ...
    int64_t x = int64_t(select(FSocket + 1, NULL, &FDset, NULL, &TimeV)); //<-Ignore this warning in 64bit Visual Studio
    if (x==(int64_t)SOCKET_ERROR)
    {
        LastTcpError = GetLastSocketError();
        x=0;
    }
...
bool TMsgSocket::CanRead(int Timeout)
{
    ...
    int64_t x = (int64_t)select(FSocket + 1, &FDset, NULL, NULL, &TimeV); //<-Ignore this warning in 64bit Visual Studio
    if (x == (int64_t)SOCKET_ERROR)
    {
       LastTcpError = GetLastSocketError();
       x=0;
