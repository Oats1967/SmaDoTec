...
uint8_t crc_calculateCRC8(const uint8_t* crc_DataPtr, uint32_t crc_Length, uint8_t crc_StartValue8)
{
  ...
  if(NULL != crc_DataPtr)
  {
    ...
    {
      cb = crc_DataPtr[i]; /* PRQA S 0492 *//* Suppress Level 5 Warning: Array subscripting applied to a function parameter declared as a pointer. - crc_Length is given as param */
      for(j=0; j<8; j++)
      {
         if((crc & 0x80) != (cb & 0x80))
         {
